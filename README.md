...
- Корзина кейсов с возможностью добавить несколько — сохраняется в LocalStorage между сессиями
...
├── frontend/
│   └── src/
│       ├── pages/      # Home, CasePage, Cart, Login, Register, Inventory, Profile, Admin
│       ├── components/ # Navbar
│       └── utils/      # api.ts (axios + LocalStorage + cartStorage), AuthContext
└── DB/
